        ...
        <div className="space-y-3">
          {filteredNotifications.length === 0 ? (
            ...
            filteredNotifications.map((notification) => (
              <div
                key={notification.id}
                className={`bg-white rounded-xl border-l-4 p-5 shadow-sm transition-all ${getSeverityColor(
                  notification.severity
                )} ${!notification.read ? 'border-l-4' : 'border-l-4 opacity-75'} ${
                  notification.type === 'Downtime' ? 'ring-2 ring-red-500 ring-opacity-50' : ''
                }`}
              >
